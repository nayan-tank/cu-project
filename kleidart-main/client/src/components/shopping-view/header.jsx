  ...
  return (
    <nav className="flex flex-col mb-3 lg:mb-0 lg:items-center gap-6 lg:flex-row">
      {shoppingViewHeaderMenuItems.map((menuItem) => (
        <Label
          onClick={() => {
            if (menuItem.label === "Gallery") {
              navigate("/shop/gallery");
            } else {
          ...
          key={menuItem.id}
        >
          {menuItem.label === "Search" ? <Search /> : menuItem.label}
        </Label>
      ))}
    </nav>
  );
  ...
  return (
    <div className="flex lg:items-center lg:flex-row flex-col gap-4">
      {isAuthenticated ? (
        ...
        <Button
          className="bg-black text-white lg:mt-0 mt-4"
